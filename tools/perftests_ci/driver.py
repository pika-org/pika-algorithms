...
@args.arg(
    "--cmake-only", action="store_true", help="only execute CMake but do not build"
)
...
def build(
    build_type, environment, target, source_dir, build_dir, install_dir, cmake_only
):
    import build
...
if buildinfo:
    ...
    @args.arg(
        "--perftests-only", action="store_true", help="only run perftests binaries"
    )
    @args.arg("--verbose-ctest", action="store_true", help="run ctest in verbose mode")
    @args.arg(
        "--examples-build-dir",
    ...
    def test(
        run_mpi_tests, perftests_only, verbose_ctest, examples_build_dir, build_examples
    ):
        import test
...
if buildinfo:
    ...
    @args.arg("--local", default=False, help="run without slurm")
    @args.arg(
        "--run_output",
        "-o",
        required=True,
...
@args.arg("--output", "-o", required=True, help="output directory")
@args.arg("--input", "-i", required=True, nargs="+", help="any number of input files")
@args.arg(
    "--date",
    "-d",
    default="job",
