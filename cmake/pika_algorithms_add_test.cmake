...
function(pika_algorithms_add_test category name)
  ...
  if(NOT ${name}_THREADS)
    set(${name}_THREADS 1)
  elseif(PIKA_ALGORITHMS_WITH_TESTS_MAX_THREADS GREATER 0
         AND ${name}_THREADS GREATER PIKA_ALGORITHMS_WITH_TESTS_MAX_THREADS
  )
    set(${name}_THREADS ${PIKA_ALGORITHMS_WITH_TESTS_MAX_THREADS})
  endif()
  ...
  if(${PIKA_ALGORITHMS_WITH_TESTS_DEBUG_LOG})
    set(args
        ${args}
        "--pika:debug-pika-log=${PIKA_ALGORITHMS_WITH_TESTS_DEBUG_LOG_DESTINATION}"
    )
  endif()

  if(${PIKA_ALGORITHMS_WITH_PARALLEL_TESTS_BIND_NONE}
     AND NOT run_serial
  ...
  # Only real tests, i.e. executables ending in _test, link to
  # pika_algorithms_testing
  if(TARGET ${${name}_EXECUTABLE}_test AND ${name}_TESTING)
    target_link_libraries(
      ${${name}_EXECUTABLE}_test PRIVATE pika_algorithms_testing
    )
  endif()

  if(TARGET ${${name}_EXECUTABLE}_test AND ${name}_PERFORMANCE_TESTING)
    target_link_libraries(
      ${${name}_EXECUTABLE}_test PRIVATE pika_algorithms_performance_testing
    )
  endif()

endfunction(pika_algorithms_add_test)

function(pika_algorithms_add_test_target_dependencies category name)
  ...
  pika_algorithms_add_pseudo_target(${category}.${name})
  # Make pseudo-targets depend on master pseudo-target
  pika_algorithms_add_pseudo_dependencies(${category} ${category}.${name})
  ...
  if(${name}_PSEUDO_DEPS_NAME)
    ...
    pika_algorithms_add_pseudo_dependencies(${category}.${name} ${name}${_ext})
  endif()
endfunction(pika_algorithms_add_test_target_dependencies)

# To add test to the category root as in tests/regressions/ with correct name
function(pika_algorithms_add_test_and_deps_test category subcategory name)
  if("${subcategory}" STREQUAL "")
    pika_algorithms_add_test(tests.${category} ${name} ${ARGN})
    pika_algorithms_add_test_target_dependencies(
      tests.${category} ${name} ${ARGN}
    )
  else()
    pika_algorithms_add_test(tests.${category}.${subcategory} ${name} ${ARGN})
    pika_algorithms_add_test_target_dependencies(
      tests.${category}.${subcategory} ${name} ${ARGN}
    )
  endif()
endfunction(pika_algorithms_add_test_and_deps_test)
...
function(pika_algorithms_add_unit_test subcategory name)
  pika_algorithms_add_test_and_deps_test(
    "unit" "${subcategory}" ${name} ${ARGN} TESTING
  )
endfunction(pika_algorithms_add_unit_test)

function(pika_algorithms_add_regression_test subcategory name)
  # ARGN needed in case we add a test with the same executable
  pika_algorithms_add_test_and_deps_test(
    "regressions" "${subcategory}" ${name} ${ARGN} TESTING
  )
endfunction(pika_algorithms_add_regression_test)

function(pika_algorithms_add_performance_test subcategory name)
  pika_algorithms_add_test_and_deps_test(
    "performance" "${subcategory}" ${name} ${ARGN} RUN_SERIAL
    PERFORMANCE_TESTING
  )
endfunction(pika_algorithms_add_performance_test)

function(pika_algorithms_add_example_test subcategory name)
  pika_algorithms_add_test_and_deps_test(
    "examples" "${subcategory}" ${name} ${ARGN}
  )
endfunction(pika_algorithms_add_example_test)

# To create target examples.<name> when calling make examples need 2 distinct
# rules for examples and tests.examples
function(pika_algorithms_add_example_target_dependencies subcategory name)
  ...
  if(NOT ${name}_DEPS_ONLY)
    # Add a custom target for this example
    pika_algorithms_add_pseudo_target(examples.${subcategory}.${name})
  endif()
  # Make pseudo-targets depend on master pseudo-target
  pika_algorithms_add_pseudo_dependencies(
  ...
  # Add dependencies to pseudo-target
  pika_algorithms_add_pseudo_dependencies(
    examples.${subcategory}.${name} ${name}
  )
endfunction(pika_algorithms_add_example_target_dependencies)
