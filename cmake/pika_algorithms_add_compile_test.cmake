...
function(pika_algorithms_add_compile_test category name)
...
endfunction(pika_algorithms_add_compile_test)

function(pika_algorithms_add_compile_test_target_dependencies category name)
  pika_algorithms_add_pseudo_target(${category}.${name})
  # make pseudo-targets depend on master pseudo-target
  pika_algorithms_add_pseudo_dependencies(${category} ${category}.${name})
endfunction(pika_algorithms_add_compile_test_target_dependencies)

# To add test to the category root as in tests/regressions/ with correct name
function(pika_algorithms_add_test_and_deps_compile_test category subcategory
         name
)
  if("${subcategory}" STREQUAL "")
    pika_algorithms_add_compile_test(tests.${category} ${name} ${ARGN})
...
endfunction(pika_algorithms_add_test_and_deps_compile_test)

function(pika_algorithms_add_unit_compile_test subcategory name)
  pika_algorithms_add_test_and_deps_compile_test(
    "unit" "${subcategory}" ${name} ${ARGN}
  )
endfunction(pika_algorithms_add_unit_compile_test)

function(pika_algorithms_add_regression_compile_test subcategory name)
  pika_algorithms_add_test_and_deps_compile_test(
    "regressions" "${subcategory}" ${name} ${ARGN}
  )
endfunction(pika_algorithms_add_regression_compile_test)

function(pika_algorithms_add_headers_compile_test subcategory name)
...
endfunction(pika_algorithms_add_headers_compile_test)

function(pika_algorithms_add_header_tests category)
  set(options NOLIBS)
  ...
  foreach(header ${${category}_HEADERS})
    ...
    if(${detail_pos} EQUAL -1 AND ${exclude_pos} EQUAL -1)
      ...
      file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/${full_test_file}
           "#include <${relpath}>\n" "#ifndef PIKA_ALGORITHMS_MAIN_DEFINED\n"
           "int main() { return 0; }\n" "#endif\n"
      )
      ...
      pika_algorithms_add_headers_compile_test(
        ...
        DEPENDENCIES ${${category}_DEPENDENCIES} pika_algorithms_private_flags
                     pika_algorithms_public_flags ${_additional_flags}
      )
  ...
  file(WRITE ${all_headers_test_file}
       ${all_headers} "#ifndef PIKA_ALGORITHMS_MAIN_DEFINED\n"
       "int main() { return 0; }\n" "#endif\n"
  )
