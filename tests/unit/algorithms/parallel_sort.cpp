...
void test1()
{
    ...
    pika::sort(pika::execution::par, V1.begin(), V1.end(), comp);

    for (unsigned i = 1; i < NElem; i++)
    {
        PIKA_TEST(V1[i - 1] <= V1[i]);
    }
...
void test2()
{
    ...
    PIKA_TEST(A[998] == 0 && A[999] == 0 && A[1000 + NELEM] == 0 &&
        A[1001 + NELEM] == 0);
    ...
    PIKA_TEST(A[998] == 999999999 && A[999] == 999999999 &&
...
void test3()
{
    ...
    pika::sort(pika::execution::par, V1.begin(), V1.end(), compare());
...
void test4()
{
    ...
    pika::sort(pika::execution::par, p64_1, p64_1 + NELEM, cmp64);

    for (unsigned i = 1; i < NELEM; i++)
    {
        PIKA_TEST(p64_1[i - 1] <= p64_1[i]);
    }
    ...
    {
        PIKA_TEST(p64_1[i] <= p64_2[i]);
    }

    // 32 bits elements
    V1 = V2 = V3;
    std::uint32_t* p32_1 = reinterpret_cast<std::uint32_t*>(&V1[0]);
    pika::sort(pika::execution::par, p32_1, p32_1 + (NELEM << 1), cmp32);

    for (unsigned i = 1; i < (NELEM << 1); i++)
    {
        PIKA_TEST(p32_1[i - 1] <= p32_1[i]);
    }
    ...
    {
        PIKA_TEST(p32_1[i] == p32_2[i]);
    }

    // 16 bits elements
    V1 = V2 = V3;
    std::uint16_t* p16_1 = reinterpret_cast<std::uint16_t*>(&V1[0]);
    pika::sort(pika::execution::par, p16_1, p16_1 + (NELEM << 2), cmp16);
    for (unsigned i = 1; i < (NELEM << 2); i++)
    {
        PIKA_TEST(p16_1[i - 1] <= p16_1[i]);
    }
    ...
    for (unsigned i = 1; i < (NELEM << 2); i++)
    {
        PIKA_TEST(p16_1[i] == p16_2[i]);
    };

    // 8 bits elements
    V1 = V2 = V3;
    std::uint8_t* p8_1 = reinterpret_cast<std::uint8_t*>(&V1[0]);
    pika::sort(pika::execution::par, p8_1, p8_1 + (NELEM << 3), cmp8);
    for (unsigned i = 1; i < (NELEM << 3); i++)
    {
        PIKA_TEST(p8_1[i - 1] <= p8_1[i]);
    }
