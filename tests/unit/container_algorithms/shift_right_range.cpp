...
void test_shift_right(IteratorTag)
{
    ...
    // shift by zero should have no effect
    pika::ranges::shift_right(c, 0);
    PIKA_TEST(std::equal(std::begin(c), std::end(c), std::begin(d)));

    // shift by a negative number should have no effect
    pika::ranges::shift_right(c, -4);
    PIKA_TEST(std::equal(std::begin(c), std::end(c), std::begin(d)));

    std::size_t n = (std::rand() % (std::size_t) ARR_SIZE) + 1;
    pika::ranges::shift_right(c, n);

    std::move_backward(std::begin(d), std::end(d) - n, std::end(d));

    // verify values
    PIKA_TEST(std::equal(std::begin(c) + n, std::end(c), std::begin(d) + n));

    // ensure shift by more than n does not crash
    pika::ranges::shift_right(c, (std::size_t)(ARR_SIZE + 1));
}
...
void test_shift_right(ExPolicy policy, IteratorTag)
{
    ...
    // shift by zero should have no effect
    pika::ranges::shift_right(policy, c, 0);
    PIKA_TEST(std::equal(std::begin(c), std::end(c), std::begin(d)));

    // shift by a negative number should have no effect
    pika::ranges::shift_right(policy, c, -4);
    PIKA_TEST(std::equal(std::begin(c), std::end(c), std::begin(d)));

    std::size_t n = (std::rand() % (std::size_t) ARR_SIZE) + 1;
    pika::ranges::shift_right(policy, c, n);
    ...
    // ensure shift by more than n does not crash
    pika::ranges::shift_right(policy, c, (std::size_t)(ARR_SIZE + 1));
}
...
void test_shift_right_async(ExPolicy policy, IteratorTag)
{
    ...
    // shift by zero should have no effect
    auto fut1 = pika::ranges::shift_right(policy, c, 0);
    fut1.wait();
    PIKA_TEST(std::equal(std::begin(c), std::end(c), std::begin(d)));

    // shift by a negative number should have no effect
    auto fut2 = pika::ranges::shift_right(policy, c, -4);
    fut2.wait();
    PIKA_TEST(std::equal(std::begin(c), std::end(c), std::begin(d)));

    std::size_t n = (std::rand() % (std::size_t) ARR_SIZE) + 1;
    auto fut3 = pika::ranges::shift_right(policy, c, n);
    fut3.wait();
