...
void test_sort1_sent(T)
{
    ...
    pika::ranges::sort(std::begin(c), sentinel<T>{rand_max_val});
...
void test_sort1_sent(ExPolicy&& policy, T)
{
    ...
    c[N - 1] = rand_max_val;
    // sort, blocking when seq, par, par_vec
    pika::ranges::sort(policy, std::begin(c), sentinel<T>{rand_max_val});
...
void test_sort1_comp_sent(ExPolicy&& policy, T, Compare comp = Compare())
{
    ...
    c[N - 1] = rand_max_val;
    // sort, blocking when seq, par, par_vec
    pika::ranges::sort(policy, std::begin(c), sentinel<T>{rand_max_val}, comp);
...
void test_sort1(ExPolicy&& policy, T)
{
    ...
    auto t = high_resolution_clock::now();
    // sort, blocking when seq, par, par_vec
    pika::ranges::sort(std::forward<ExPolicy>(policy), c);
    duration<double> elapsed = high_resolution_clock::now() - t;
...
void test_sort1_comp(ExPolicy&& policy, T, Compare comp = Compare())
{
    ...
    auto t = high_resolution_clock::now();
    // sort, blocking when seq, par, par_vec
    pika::ranges::sort(std::forward<ExPolicy>(policy), c, comp);
    duration<double> elapsed = high_resolution_clock::now() - t;
...
void test_sort2(ExPolicy&& policy, T)
{
    ...
    auto t = high_resolution_clock::now();
    // sort, blocking when seq, par, par_vec
    pika::ranges::sort(std::forward<ExPolicy>(policy), c.begin(), c.end());
    duration<double> elapsed = high_resolution_clock::now() - t;
...
void test_sort2_async(ExPolicy&& policy, T, Compare comp = Compare())
{
    ...
    auto t = high_resolution_clock::now();
    // sort, non blocking
    pika::future<void> f = pika::ranges::sort(
        std::forward<ExPolicy>(policy), c.begin(), c.end(), comp);
    f.get();
    duration<double> elapsed = high_resolution_clock::now() - t;
