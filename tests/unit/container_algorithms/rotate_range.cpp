...
void test_rotate_sent(IteratorTag)
{
    ...
    std::size_t count = 0;
    PIKA_TEST(std::equal(std::begin(c), std::end(c) - 1, std::begin(d1),
...
void test_rotate(IteratorTag)
{
    ...
    pika::ranges::rotate(c, iterator(mid));
...
void test_rotate(ExPolicy policy, IteratorTag)
{
    ...
    pika::ranges::rotate(policy, c, mid);
...
void test_rotate_async(ExPolicy p, IteratorTag)
{
    ...
    auto f = pika::ranges::rotate(p, c, mid);
    f.wait();
