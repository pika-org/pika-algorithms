...
void test_replace_copy_sent()
{
    ...
    pika::ranges::replace_copy(std::begin(c), sentinel<std::int16_t>{50},
...
void test_replace_copy(IteratorTag)
{
    ...
    pika::ranges::replace_copy(c, std::begin(d1), c[idx], c[idx] + 1);
...
void test_replace_copy(ExPolicy policy, IteratorTag)
{
    ...
    pika::ranges::replace_copy(policy, c, std::begin(d1), c[idx], c[idx] + 1);
...
void test_replace_copy_async(ExPolicy p, IteratorTag)
{
    ...
    auto f =
        pika::ranges::replace_copy(p, c, std::begin(d1), c[idx], c[idx] + 1);
    f.wait();
...
void test_replace_copy_exception(ExPolicy policy, IteratorTag)
{
    ...
    try
    {
        pika::ranges::replace_copy(policy,
            pika::util::make_iterator_range(
                decorated_iterator(
                    std::begin(c), []() { throw std::runtime_error("test"); }),
                decorated_iterator(std::end(c))),
...
void test_replace_copy_exception_async(ExPolicy p, IteratorTag)
{
    ...
    try
    {
        auto f = pika::ranges::replace_copy(p,
            pika::util::make_iterator_range(
                decorated_iterator(
                    std::begin(c), []() { throw std::runtime_error("test"); }),
                decorated_iterator(std::end(c))),
