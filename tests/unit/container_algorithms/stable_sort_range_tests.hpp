...
int verify_(const std::vector<IA>& A, Compare comp, double elapsed, bool print)
{
    if (A.size() < 2)
    {
...
void test_stable_sort1_sent(T)
{
    ...
    pika::ranges::stable_sort(std::begin(c), sentinel<T>{rand_max_val});
...
void test_stable_sort1_sent(ExPolicy&& policy, T)
{
    ...
    c[N - 1] = rand_max_val;
    // sort, blocking when seq, par, par_vec
    pika::ranges::stable_sort(policy, std::begin(c), sentinel<T>{rand_max_val});
...
void test_stable_sort1(T)
{
    ...
    // sort, blocking when seq, par, par_vec
    pika::ranges::stable_sort(c);
    duration<double> elapsed = high_resolution_clock::now() - t;

    bool is_sorted = (verify_(c, std::less<T>(), elapsed.count(), true) != 0);
    PIKA_TEST(is_sorted);
}
...
void test_stable_sort1(ExPolicy&& policy, T)
{
    ...
    auto t = high_resolution_clock::now();
    // sort, blocking when seq, par, par_vec
    pika::ranges::stable_sort(std::forward<ExPolicy>(policy), c);
    duration<double> elapsed = high_resolution_clock::now() - t;
...
void test_stable_sort1_comp(ExPolicy&& policy, T, Compare comp = Compare())
{
    ...
    auto t = high_resolution_clock::now();
    // sort, blocking when seq, par, par_vec
    pika::ranges::stable_sort(std::forward<ExPolicy>(policy), c, comp);
    duration<double> elapsed = high_resolution_clock::now() - t;

    bool is_sorted = (verify_(c, comp, elapsed.count(), true) != 0);
    PIKA_TEST(is_sorted);
}
...
void test_stable_sort1_async(ExPolicy&& policy, T, Compare comp = Compare())
{
    ...
    auto t = high_resolution_clock::now();
    // sort, non blocking
    auto f = pika::ranges::stable_sort(std::forward<ExPolicy>(policy), c, comp);
    f.get();
    duration<double> elapsed = high_resolution_clock::now() - t;
...
void test_stable_sort2_async(ExPolicy&& policy, T, Compare comp = Compare())
{
    ...
    auto t = high_resolution_clock::now();
    // sort, non blocking
    pika::future<void> f = pika::ranges::stable_sort(
        std::forward<ExPolicy>(policy), c.begin(), c.end(), comp);
    f.get();
    duration<double> elapsed = high_resolution_clock::now() - t;
...
void test_stable_sort1(ExPolicy&& policy, const std::string&)
{
    ...
    bool is_sorted =
        (verify_(c, std::less<std::string>(), elapsed.count(), true) != 0);
    PIKA_TEST(is_sorted);
}
