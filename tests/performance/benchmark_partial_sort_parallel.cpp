...
void function01()
{
    ...
    {
        B = A;
        pika::partial_sort(::pika::execution::par, B.begin(), B.begin() + i,
            B.end(), compare_t());
    }
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<long unsigned, std::nano> nanotime1 = end - start;
    std::cout << "pika::parallel::partial_sort :"
...
void function02()
{
    ...
    // pika::parallel::partial sort
    B = A;
    start = std::chrono::high_resolution_clock::now();
    pika::partial_sort(pika::execution::par, B.begin(), B.end(), B.end(), comp);
    end = std::chrono::high_resolution_clock::now();
...
void function03()
{
    ...
    std::cout << "\n\n";
    std::cout << "Accumulated (msec) pika::partial_sort " << ac1 / 1000000
              << std::endl;
